...
if [[ $1 == "" || $1 == "default" ]]
then
    javac -encoding UTF-8 -d target -cp "lib/*${CP_DELIM}src" src/main/java/*.java
    javac -encoding UTF-8 -d target -cp "lib/*${CP_DELIM}src" src/test/java/*.java
elif [[ $1 == "run" ]]
then
    javac -encoding UTF-8 -d target -cp "lib/*${CP_DELIM}src" "src/${MAIN//.//}.java"
